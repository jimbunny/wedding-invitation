...
class AuthorizationResource(Resource):
    ...
    def post(self, username="", password="", type="user"):
        """
        用户登录创建token
        """
        if type == "user":
            userInfo = AppUsersModel.query.filter_by(username=username).first()
            if (userInfo is None):
                return pretty_result(code.ERROR, data='', msg='user is not exit')
            # if (not userInfo.confirmed):
            #     return pretty_result(code.ERROR, data='', msg='请验证邮箱')
            else:
                if (AppUsersModel.check_password(AppUsersModel, userInfo.password, password)):
                    login_time = int(time.time())
                    userInfo.login_time = login_time
                    AppUsersModel.update(AppUsersModel)
                    user_id = userInfo.id
                    token, refresh_token = self._generate_tokens(user_id)
                    return pretty_result(code.OK, data={'access_token': token, 'refresh_token': refresh_token}, msg='Login successful!')
                else:
                    return pretty_result(code.ERROR, data='', msg='Password is not correct!')
        else:
            userInfo = AppUsersModel.query.filter_by(facebook=username).first()
            login_time = int(time.time())
            userInfo.login_time = login_time
            AppUsersModel.update(AppUsersModel)
            user_id = userInfo.id
        ...
        if (auth_header):
            ...
            if (not auth_tokenArr or auth_tokenArr[0] != 'JWT' or len(auth_tokenArr) != 2):
                ...
                if not isinstance(payload, str):
                    user = AppUsersModel.get(AppUsersModel, payload['data']['id'])
                    if (user is None):
                        msg = '找不到该用户信息'
                    else:
