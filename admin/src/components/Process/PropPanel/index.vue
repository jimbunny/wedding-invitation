...
export default {
  ...
  methods: {
    ...
    initFormOperates(target){
      ...
      const formItems = this.$store.state.approval.formItemList.filter(t => t.cmpType !== 'custom')
      format(formItems)
      return res
    },
    ...
    initConditionNodeData(){
      ...
      this.pconditions = JSON.parse(JSON.stringify(this.$store.state.approval.processConditions));
